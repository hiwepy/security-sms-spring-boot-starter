...
class SecuritySmsAutoConfigurationTest {
    ...
    @Test
    @DisplayName("Auto-configuration loads when 'spring.security.sms.enabled=true'")
    void testLoadsWhenEnabledPropertySet() {
        runner.withUserConfiguration(SecuritySmsAutoConfiguration.class)
                .withPropertyValues("spring.security.sms.enabled=true")
                .run(context -> assertThat(context).hasSingleBean(SecuritySmsAutoConfiguration.class));
    }
    ...
    void testNotLoadedWhenPropertyAbsent() {
        runner.withUserConfiguration(SecuritySmsAutoConfiguration.class)
                .run(context -> assertThat(context).doesNotHaveBean(SecuritySmsAutoConfiguration.class));
    }
}
